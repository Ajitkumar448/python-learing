...
print(password_checker("abcde123"))
print(password_checker("abcdeaaa"))

def login_system(user, password):# login system that checks if the username is "admin" and the password is "1234" and returns a message accordingly
...
print(login_system("admin", "1234"))
print(login_system("admin", "wrongpassword"))

def quiz():
    answer =input("enter ur answer: ")
    if answer.lower() =="ajit":
        return"correct answer"
    elif answer == "":
        return"plz enter ur answer"
    else:
...
def check_palindrome(text):# PALINDROME check text
...
def check_palindrome(number):# palindrome check number
...
original_number = int(input("Enter a number: ")) # to find reverse
...
def factorial(number):# factorial of a number
    fact = 1
...
def check_prime(number):# to check if a number is prime or not
...
def fibonacci(limit): # find fibonacci series up to a certain limit
...
def calculate_area(l,w):
    return (l*w)
rectangle_area =calculate_area(5,7)
